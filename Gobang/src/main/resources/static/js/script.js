...
websocket.onmessage = function(event) {
    ...
    if (resp.message == 'gameReady') {
        ...
        alert("当前用户已登录,请使用其他账号登录");
        location.replace("/login.html");
    }else if(resp.meesage == 'notLogin'){
        alert("当前用户尚未登录");
        location.replace("/login.html");
    }else{
        console.log("收到了非法响应" + resp.reason);
    }
    // } else if (resp.message == 'repeatConnection') {
...
function initGame() {
    ...
    websocket.onmessage = function(event) {
        ...
        if (resp.winner != 0 && resp.winner != -1) {
            ...
            let backBtn = document.querySelector("#but");
